...
from Global import CARS_INFO, CELL_RATIO, TIME_SLICE, TIMES
...
from functions import multi_probility_test, accumulator
...
class CellularHandler(object):
    ...
    def driver(self, route_id):
        ...
        for index, path in enumerate(self.route_list[route_id].path_list):
            ...
            count = 0
            print
            "[driver] path index is %s, amount is %s" % (index, amount)
            while (count < amount):
        ...
        "volume is %s" % avg_vol
        print
        "velocity is %s , %s" % (avg_vel, reocord_velocity)
